...
"         \ }
"
" let s:denite_win_width_percent = 0.2
" let s:denite_win_height_percent = 0.2
"
" nnoremap <F3> :<C-u> Defx -columns=icons:indent:filename:type -resume -toggle -floating-preview  -buffer-name=tab`tabpagenr()`<CR>
" " nnoremap <leader>ft :<C-u>Defx -columns=icons:indent:filename:type -resume -toggle -buffer-name=tab`tabpagenr()`<CR>
" " nnoremap <leader>ft :<C-u>Defx -columns=icons:indent:filename:type -resume -buffer-name=tab`tabpagenr()` -search=`expand('%:p')`<CR>
"
...
"   nnoremap <silent><buffer><expr> =     defx#do_action('preview')
